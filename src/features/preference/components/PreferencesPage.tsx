// src/features/preferences/components/PreferencesPage.tsx

/**
 * Preferences Management Component
 * 
 * Complete preferences management interface with table view, CRUD dialogs,
 * confirmation modals, and preference detail viewing.
...
import type { Preference, CreatePreferenceDto, UpdatePreferenceDto } from '../types';

// Form validation schemas
const createPreferenceSchema = z.object({
  name: z.string().min(1, 'Preference name is required').max(255, 'Name must be less than 255 characters'),
...
type UpdatePreferenceForm = z.infer<typeof updatePreferenceSchema>;
...
const PreferencesPage: React.FC = () => {
  ...
  // Local state for editing preference
  const [editingPreference, setEditingPreference] = useState<Preference | null>(null);

  // Navigation hook
  const navigate = useNavigate();

  const createForm = useForm<CreatePreferenceForm>({
  ...
  const updateForm = useForm<UpdatePreferenceForm>({
  ...
  useEffect(() => {
    if (error.list) {
      toast.error('Error loading preferences', { description: error.list });
      clearError('list');
    }
  ...
  useEffect(() => {
    if (error.create) {
      toast.error('Error creating preference', { description: error.create });
      clearError('create');
    }
  ...
  useEffect(() => {
    if (error.update) {
      toast.error('Error updating preference', { description: error.update });
      clearError('update');
    }
  ...
  useEffect(() => {
    if (error.delete) {
      toast.error('Error deleting preference', { description: error.delete });
      clearError('delete');
    }
  ...
  const handleRefresh = async () => {
    try {
      ...
      toast.success('Preferences refreshed', { description: 'Preference list updated successfully.' });
    } catch {
  ...
  const handleViewClick = (preference: Preference) => {
    // Implement navigation or detail modal if needed
     navigate(`/preferences/${preference.id}`);
  };

  const handleDeleteClick = (preference: Preference) => {
    setDeletingId(preference.id);
  };

  const handleCreateSubmit = async (data: CreatePreferenceForm) => {
    try {
      ...
      toast.success('Preference created', { description: `Preference "${data.name}" created successfully.` });
      createForm.reset();
      closeCreateModal();
    } catch {
      // Handled by useEffect
  ...
  const handleUpdateSubmit = async (data: UpdatePreferenceForm) => {
    ...
    try {
      ...
      toast.success('Preference updated', { description: `Preference "${data.name}" updated successfully.` });
      updateForm.reset();
      closeEditModal();
  ...
  const handleDeleteConfirm = async () => {
    ...
    const preference = preferences.find(p => p.id === ui.deletingId);
    try {
      await deletePreference(ui.deletingId);
      toast.success('Preference deleted', { description: `Preference "${preference?.name || 'Unknown'}" deleted successfully.` });
      setDeletingId(null);
    } catch {
      // Handled by useEffect
    }
  ...
  const deletingPreference = preferences.find(p => p.id === ui.deletingId);
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Manage your organization's preferences</p>
            </div>
        ...
        <CardContent className="p-0">
          {loading.list ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-sm text-gray-500 mb-4">Get started by creating your first preference.</p>
            ...
            <Table>
              ...
              <TableBody>
                {preferences.map(preference => (
                  <TableRow key={preference.id} className="hover:bg-gray-50">
                    <TableCell className="font-medium">{preference.name}</TableCell>
                    <TableCell className="text-gray-600">
                      {preference.slug || <span className="text-gray-400 italic">No slug</span>}
                    </TableCell>
                    <TableCell className="text-gray-600">
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end space-x-2">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleViewClick(preference)}
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleDeleteClick(preference)}
      ...
      <Dialog open={ui.isCreateModalOpen} onOpenChange={handleCloseCreateDialog}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Add a new preference to your organization. The slug field is optional and will be used for URL-friendly identification.
            </DialogDescription>
          </DialogHeader>
          <Form {...createForm}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Enter preference name" {...field} disabled={loading.create} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
      ...
      <Dialog open={ui.isEditModalOpen} onOpenChange={handleCloseEditDialog}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>Update the preference details. Changes will be saved immediately.</DialogDescription>
          </DialogHeader>
          <Form {...updateForm}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Preference Name *</FormLabel>
                    <FormControl>
                      <Input placeholder="Enter preference name" {...field} disabled={loading.update} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
      ...
      <AlertDialog open={!!ui.deletingId} onOpenChange={handleCloseDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the preference{' '}
              <span className="font-semibold">"{deletingPreference?.name}"</span> and all associated data.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
