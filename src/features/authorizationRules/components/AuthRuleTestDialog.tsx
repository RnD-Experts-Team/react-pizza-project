...
export const AuthRuleTestDialog: React.FC<AuthRuleTestDialogProps> = ({
  ...
  return (
    <Dialog open={true} onOpenChange={handleClose}>
      <DialogContent 
        className="w-[95vw] max-w-sm sm:max-w-md lg:max-w-lg max-h-[90vh] overflow-y-auto bg-card border border-border"
        style={{
          boxShadow: 'var(--shadow-realistic)'
        }}
      >
        <DialogHeader className="pb-3 sm:pb-4">
          <DialogTitle className="text-lg sm:text-xl text-center sm:text-left text-foreground">
        ...
        <div className="space-y-3 sm:space-y-4 px-1">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              className="font-mono text-xs sm:text-sm min-h-[3rem] sm:min-h-[4rem] resize-none bg-input border-border text-foreground"
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <Input
                ...
                className="text-xs sm:text-sm font-mono flex-1 bg-input border-border text-foreground"
              />
              <Button 
                onClick={handleTest} 
                disabled={!pathDsl.trim() || !testPath.trim() || isTesting}
                size="sm"
                className="w-full sm:w-auto min-w-[4rem] text-sm disabled:bg-muted disabled:opacity-60"
                style={{
                  backgroundColor: (!pathDsl.trim() || !testPath.trim() || isTesting) ? undefined : 'var(--primary)',
          ...
          {/* Test Result */}
          {testResult && (
            <div className="space-y-3 p-3 sm:p-4 border rounded-md bg-muted border-border">
              <div className="flex items-center gap-2">
                {testResult.matches ? (
                  ...
                  <XCircle 
                    className="h-4 w-4 sm:h-5 sm:w-5 flex-shrink-0 text-destructive" 
                  />
                )}
              ...
              <div className="space-y-2 sm:space-y-3">
                <div>
                  ...
                  <div className="font-mono text-xs sm:text-sm p-2 sm:p-3 rounded mt-1 break-all bg-card border border-border text-foreground">
          ...
          <div className="flex justify-end gap-2 pt-2 sm:pt-3">
            <Button 
              variant="outline" 
              onClick={handleClose}
              className="w-full sm:w-auto min-w-[5rem] text-sm bg-secondary text-secondary-foreground border-border"
            >
              Close
            </Button>
          </div>
